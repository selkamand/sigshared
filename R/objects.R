...
#' @param ls_signatures_minified List. A list of signatures (in model) rendered as minified ggplots. Names should be concatenations of each signature and its aetiology. If NULL, we assume no signature model could be fit. See [sigvis::sig_visualise_minified()].
...
#' @param similar_sample_disease_donut ggplot (optional). A donut plot of the diseases of the top N similar samples. Default is \code{NULL}.
...
#'   similar_sample_disease_donut = ggplot_obj5,
#'   umap = ggplot_obj6,
...
sigstory_visualisation <- function(
  ...
  if(!is.null(ls_signatures_minified)) assertions::assert_list(ls_signatures_minified)
  if(!is.null(similar_sample_disease_donut)) assertions::assert_class(similar_sample_disease_donut, class = "ggplot")
  if(!is.null(umap)) assertions::assert_class(umap, class = "ggplot")
  if(!is.null(stability)) assertions::assert_class(stability, class = "ggplot")
  ...
  ls <- list(
    ...
    similar_sample_disease_donut = similar_sample_disease_donut,  # Donut plot of diseases of top similar samples
